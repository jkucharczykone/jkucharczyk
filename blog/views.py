...
def post_detail(request, year, month, day, post):
    ...
    similar_posts = Post.objects.all().filter(status='published',tags__in=post_tags_ids).exclude(id=post.id)
...
def post_share(request, post_id):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            send_mail(subject, message, 'quasi@mail.ee',["{}".format(cd['to'])],fail_silently=False)
            sent=True
    else:
        form = EmailPostForm()
...
def post_search(request):
    ...
    if 'query' in request.GET:
        ...
        if form.is_valid():
            ...
            search_vector = SearchVector("title",wieght='A') + SearchVector("body",weight='B')
    ...
    return render(request, 'blog/post/search.html', {'form': form,'query':query, 'results': results,'total_results': len(results)})
# def post_search(request):
#     form = SearchForm()
#     query = None
#     results = []
